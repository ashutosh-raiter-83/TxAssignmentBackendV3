...
public class SentCommandController_ListTest : WebAppFactoryFixture
{
    public SentCommandController_ListTest(WebAppFactory factory) : base(factory)
    {
    }
    ...
    public async Task WhenUnauthenticated_ShouldReceiveUnauthorized()
    {
        ...
        var response = await client.GetAsync(
            $"/technician-user/robot/{robotId}/command/sent"
        );
    ...
    public async Task WhenRobotExists_ShouldSucceed()
    {
        ...
        var responseBody = await response.Content.ReadFromJsonAsync<ListSentCommandResponse>();
        responseBody.Should().NotBeNull();
        responseBody.SentCommands.Should().ContainSingle()
            .Which.Should().BeEquivalentTo(sentCommand);
    }

    [Fact]
    public async Task WhenMultipleSentCommands_ShouldSortByMostRecentSent()
    {
        var robotId = new Fixture().Create<string>();
        var username = new Fixture().Create<string>();
        var password = new Fixture().Create<string>();
        ...
        var listedA = responseBody.SentCommands.Should()
            .Contain(item => item.CommandId == sentCommandA.CommandId)
            .Which;

        var listedB = responseBody.SentCommands.Should()
            .Contain(item => item.CommandId == sentCommandB.CommandId)
            .Which;

        listedA.Should().BeEquivalentTo(sentCommandA);
        listedB.Should().BeEquivalentTo(sentCommandB);

        responseBody.SentCommands.IndexOf(listedA)
            .Should().BeGreaterThan(responseBody.SentCommands.IndexOf(listedB));
    }

    [Fact]
    public async Task WhenMultipleRobotAndSentCommands_ShouldReturnSentCommandsOfSpecifiedRobot()
    {
        ...
        var client = Factory.CreateTechnicianClient();
        var response = await client.GetAsync(
            $"/technician-user/robot/{robotIdX}/command/sent"
        );
        response.EnsureSuccessStatusCode();
        var responseBody = await response.Content.ReadFromJsonAsync<ListSentCommandResponse>();
        responseBody.Should().NotBeNull();

        responseBody.SentCommands.Should()
            .Contain(item => item.CommandId == sentCommandXA.CommandId)
            .Which
            .Should().BeEquivalentTo(sentCommandXA);

        responseBody.SentCommands.Should()
            .Contain(item => item.CommandId == sentCommandXB.CommandId)
            .Which
            .Should().BeEquivalentTo(sentCommandXB);
